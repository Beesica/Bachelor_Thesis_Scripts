...
savepath = '/net/store/nbp/projects/wd_ride_village/wd_room/John';
cd('/net/store/nbp/projects/wd_ride_village/wd_room/John');
rec_vill = readtable('desc_files_streams.csv');
...
cd(basepath);
%%
subjects = [39, 40, 41, 43, 44, 45, 46, 47, 48, 50, 53, 54, 55, 56, 57, ...
    58, 59, 60];
...
cd('/net/store/nbp/projects/wd_ride_village/recordedData/wd_room/');

% get the corresponding file name
cntfile = dir(string(s)+'_room1_'+'*'+'.xdf');
...
% find matching uid
for i=1 : height(rec_vill(:,2)) % go through each of the rows
    if char(rec_vill{i,2}) == filename 
        idx = i;
        break
    end
end
...
trgpath = '/net/store/nbp/projects/wd_ride_village/wd_room/John/trigger_files';
